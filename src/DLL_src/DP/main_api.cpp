...
int tfm_BPR =0, 
int tfm_point_queue = 1;
int tfm_newells_model = 2;
...
//
	...
	float GetTravelTimeByMin(int starting_time, int time_interval, int TrafficModelFlag)  // DayNo =-1: unknown day
	{
		...
		//myTRACE(m_SimulationHorizon < m_LinkMOEAry.size());
		...
		{
			myTRACE("Error, link %d", this ->m_LinkNo );
			for(t=starting_time; t< time_end; t++)
			{
...
void BuildPhysicalNetwork(int DayNo, int CurZoneID, int TraffcModelFlag, bool bUseCurrentInformation = false, double CurrentTime = 0);
...
float g_GetPrivateProfileFloat( string section, string key, float def_value, string filename,bool print_out=false);

int g_GetPrivateProfileInt( string section, string key, int def_value, string filename, bool print_out=false);
...
extern string g_timestamp_HHMM_convertor(float time1);
...
extern char g_DTASettingFileName[_MAX_PATH];
...
extern std::string String2StdString(string str);


extern void g_AllocateDynamicArrayForAgents();
extern bool AddPathToAgent(DTAAgent * pAgent, std::vector<int> path_node_sequence);
...
extern string g_GetUsedMemoryDataInMB();
...
char g_DTASettingFileName[_MAX_PATH];
...
int g_TrafficFlowModelFlag = 0; // tfm_BPR;
...
string g_GetUsedMemoryDataInMB()
{
	MEMORYSTATUSEX statex;
...
std::string String2StdString(string str)
{	
...
std::string GetTimeClockString(int time)
{
	...
	return (String2StdString(time_str));
}
...
void g_ProgramTrace(string str)
{
		cout << str << endl;
	//	getchar();
...
int g_GetPrivateProfileInt(string section, string key, int def_value, string filename, bool print_out)
{
	int value = def_value;

	CCSVParser parser_node;
	if (parser_node.OpenCSVFile(filename))
	{
		int i = 0;
		while (parser_node.ReadRecord())
		{
			...
			parser_node.GetValueByFieldName("section", section_name);
			parser_node.GetValueByFieldName("key", key_name);
			parser_node.GetValueByFieldName("value", value);

			if (section_name == section && key_name == key)
			{
				return value;
			}
...
float g_GetPrivateProfileFloat(string section, string key, float def_value, string filename, bool print_out)
{
...
string g_timestamp_HHMM_convertor(int time_stamp_in_min)
{
	string str;
	...
	if (hour < 10)
		sprintf(x_str,"0%d:%02d", hour, min);
	else
		sprintf(x_str, "%2d:%02d", hour, min);
...
void DTANetworkForSP::BuildPhysicalNetwork(int DayNo, int CurrentZoneNo, int TraffcModelFlag, bool bUseCurrentInformation, double CurrentTime)  // for agent based
{
	...
	for (li = 0; li < g_LinkVector.size(); li++)
	{
		...
		{
			myTRACE(m_AdjLinkSize >= m_OutboundSizeAry[ToNo]);
...
// time -dependent label correcting algorithm with deque implementation
{
	...
	{
		myTRACE(LinkSize < MAX_NODE_SIZE_IN_A_PATH - 1);
...
bool g_VehicularSimulation(int DayNo, double CurrentTime, int meso_simulation_time_interval_no, int global_Traffic_Flow_Model_Flag)
{
		...
		if (pAgent->m_bLoaded == false)  // not being loaded
		{
			...
			if (pAgent->m_NodeSize >= 2)  // with physical path
			{
				...
				if (p_link != NULL)
				{
					...
					int period_no = 0;
					if (p_link->m_traffic_flow_model == 0)
...
#pragma omp parallel for
	for (int li = 0; li < link_size; li++)
	{
		...
		while (pLink->LoadingBuffer.size() > 0 && pLink->GetNumberOfLanes(DayNo, CurrentTime) > 0.01)  // no load Agent into a blocked link
		{
			...
			if (pLink->m_traffic_flow_model == 1 || (pLink->m_AgentSpaceCapacity <= 2 /* micro simulation */ && AvailableSpaceCapacity >= pLink->m_AgentSpaceCapacity) || AvailableSpaceCapacity >= max(2, pLink->m_AgentSpaceCapacity*(1 - g_MaxDensityRatioForAgentLoading)))  // at least 10% remaining capacity or 2 Agent space is left
			{
...
#pragma omp parallel for
	for (int li = 0; li < link_size; li++)
	{
		...
		while (pLink->EntranceQueue.size() > 0)  // if there are Agents in the entrance queue
		{
			...
			{
				myTRACE("Time %f, PlannedArrivalTime = %f,  CurrentTime = %f, \n",
		...
		if (pLink->m_traffic_flow_model >= 2 && g_LinkTypeMap[pLink->m_link_type].IsFreeway())  // apply spatial link in capacity for spatial queue models( with spillback and shockwave)
		{
			...
			if (pLink->m_traffic_flow_model == tfm_newells_model && g_LinkTypeMap[pLink->m_link_type].IsFreeway())  // Newell's model on freeway only
			{
				if (meso_simulation_time_interval_no >= pLink->m_BackwardWaveTimeInSimulationInterval) /// we only apply backward wave checking after the simulation time is later than the backward wave speed interval
				{
					...
					myTRACE(t_residual_minus_1 >= 0);
					myTRACE(t_residual_minus_1 < MAX_TIME_INTERVAL_ADCURVE);
		...
		while (incoming_link_count < IncomingLinkSize)
		{
			...
			while ((pLink->ExitQueue.size() > 0 && Agent_out_count > 0))  // either condition 1 or condition 2 is met
			{
				...
				if (Agent_id == Agent_id_trace)
					myTRACE("simulation link sequence no. %d", g_AgentMap[Agent_id]->m_SimLinkSequenceNo);
				...
				if (g_AgentMap[Agent_id]->m_SimLinkSequenceNo < number_of_links - 1)  // not reach destination yet
				{
					...
					if (p_Nextlink == NULL)
					{
						myTRACE("Error at Agent %d,", Agent_id);
						myTRACE(p_Nextlink != NULL);
					}
					...
					if (bNextlinkCapacity) // if there is available spatial capacity on next link, then move to next link, otherwise, stay on the current link
					{
						...
						int period_no = 0;
						if (pLink->m_traffic_flow_model == 0)
						...
						{
							myTRACE("Step 4: Time %f, Link: %d -> %d: vi %d, exit time: %f, FFTT = %f\n",
	...
	for (int li = 0; li < link_size; li++)
	{
		...
		if (meso_simulation_time_interval_no%g_number_of_intervals_per_min == 0)  // per min statistics
		{
			...
			if (time_stamp_in_min < pLink->m_LinkMOEAry.size())
			{
				...
				{
					myTRACE("step 5: statistics collection: Time %d, link %d -> %d, Cumulative arrival count %d, cumulative departure count %d \n", time_stamp_in_min, g_NodeVector[pLink->m_FromNodeNo].m_NodeID, g_NodeVector[pLink->m_ToNodeNo].m_NodeID,
...
NetworkLoadingOutput g_NetworkLoading(int TrafficFlowModelFlag = 1, int SimulationMode = 0, int Iteration = 1)  // default spatial queue // SimulationMode= default 0: UE;  1: simulation from demand; 2: simulation from Agent file
{
	...
	for (time = g_DemandLoadingStartTimeInMin; time < min(g_PlanningHorizon, g_SimululationReadyToEnd); meso_simulation_time_interval_no++)  // the simulation time clock is advanced by 0.1 seconds
	{
		...
		if (meso_simulation_time_interval_no % (5 * g_simulation_time_intervals_per_min) == 0 && bPrintOut) // every 5 min
		{
			...
			cout << "simu clock: " << hour << ":" << min << ",# of veh --Generated: " << g_Number_of_GeneratedAgents << ", Completed " << g_Number_of_CompletedAgents << ", In network: " << g_Number_of_GeneratedAgents - g_Number_of_CompletedAgents << endl;
			g_LogFile << "simulation clock: " << hour << ":" << min << ", # of Agents  -- Generated: " << g_Number_of_GeneratedAgents << ", Completed " << g_Number_of_CompletedAgents << ", In network: " << g_Number_of_GeneratedAgents - g_Number_of_CompletedAgents << endl;
		}
		...
		if (bSwitchFlag)  // for all Agents that need to switch
		{
			...
			if (pAgent->m_NodeSize >= 2)  // for feasible path
			{
				...
				for (int i = 0; i < NodeSize - 1; i++)
				{
					...
					myTRACE(pAgent->m_LinkAry[i].LinkNo < g_LinkVector.size());
...
void g_GenerateSimulationSummary(int iteration, bool NotConverged, int TotalNumOfAgentsGenerated, NetworkLoadingOutput* p_SimuOutput)
{
	...
	//g_SummaryStatFile.SetValueByFieldName("CPU Running Time", g_GetAppRunningTime());
...
void g_ReadInputFiles()
{
	...
	if (parser_link.OpenCSVFile("road_link.csv"))
	{
		...
		while (parser_link.ReadRecord())
		{
			...
			for (int link_code = link_code_start; link_code <= link_code_end; link_code++)
			{
				...
				pLink->m_traffic_flow_model = (int)traffic_flow_model_flag;
	...
	if (g_TrafficFlowModelFlag != 0 && g_TrafficFlowModelFlag != 1)
	{
	...
	for (unsigned li = 0; li < g_LinkVector.size(); li++)
	{
		...
		{  // with data string
			...
			for (int pt = 1; pt <= g_DemandTypeVector.size(); pt++)
			{
				...
				std::string str_number = String2StdString(number);
...
void g_ReadDTALiteSettings()
{
	...
	g_TrafficFlowModelFlag = (int)traffic_flow_model;
...
void g_DTALiteMultiScenarioMain()
{
	...
	case 2: 		g_LogFile << "Newell's Cumulative Flow Count Model" << endl;
...
void g_AllocateDynamicArrayForAgents()
{
	...
	{
		myTRACE("Allocate memory for %d zones and %d SP calculation intervals.\n", g_ZoneVector.size(), g_NumberOfSPCalculationPeriods);
...
bool AddPathToAgent(DTAAgent * pAgent, std::vector<int> path_node_sequence)
{
...
bool g_ReadInputAgentCSVFile(string file_name, bool bOutputLogFlag)
{
	...
	if (parser_agent.OpenCSVFile(file_name, false))
	{
		...
		while (parser_agent.ReadRecord())
		{
			...
			myTRACE("--step 1: read agent id = %d \n", agent_id);
			...
			if (g_AgentMap.find(agent_id) != g_AgentMap.end())
			{
				myTRACE("--step 2: agent id =%d found in memory, will update data\n", agent_id);
				continue;
			}
			...
			myTRACE("--step 4: read o_zone_id = %d, d_zone_id=%d \n", o_zone_id, d_zone_id);
			...
			myTRACE("--step 5: read o_node_id = %d, d_node_id=%d \n",
				o_node_id, dnode_id);
			...
			if (parser_agent.GetValueByFieldName("agent_type", agent_type) == true)
			{
				...
				myTRACE("--step 7: read agent_type = %d\n",
					agent_type);
			...
			if (parser_agent.GetValueByFieldNameRequired("time_period", time_period) == true)
			{
				...
				if (parser_agent.GetValueByFieldName("volume", volume) == true)
				{
					...
					for (int i = 0; i < number_of_agents; i++)
					{
						...
						myTRACE("--step 11: create new path\n");


						if (path_node_sequence.size() >= 2)
						{
							if (AddPathToAgent(pAgent, path_node_sequence))  // some links are not correctly defined.
								continue;
...
string g_timestamp_HHMM_convertor(float time1)
{
	// in min
	...
	if (time1 < 9999)
		sprintf_s(x_str,"%02d%02d:%02d", hour_1, min_1, sec);
	else
		sprintf_s(x_str,"9999");
...
string g_time_period_HHMM_convertor(float time1, float time2)
{
	// in min
	...
	sprintf(x_str, "%02d%02d:%02d_%02d%02d:%02d", hour_1, min_1, sec_1, hour_2, min_2, sec_2);
...
{
sprintf(g_DTASettingFileName,"./DTASettings.txt");
