...
IMPLEMENT_DYNAMIC(CSensorNetworkDataTabDlg, CDialog)

CSensorNetworkDataTabDlg::CSensorNetworkDataTabDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CSensorNetworkDataTabDlg::IDD, pParent)
{

}

CSensorNetworkDataTabDlg::CSensorNetworkDataTabDlg(std::vector<std::string> Names, std::vector<CString> default_value,
...
CSensorNetworkDataTabDlg::~CSensorNetworkDataTabDlg()
{
}

void CSensorNetworkDataTabDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	DDX_Control(pDX,IDC_GRIDLISTCTRLEX,m_ListCtrl);
}

BOOL CSensorNetworkDataTabDlg::OnInitDialog()
{
...
void CSensorNetworkDataTabDlg::SetTabText(CString s)
{
	m_TabText =  s;
}

CString CSensorNetworkDataTabDlg::GetTabText()
{
	return m_TabText;
}

BOOL CSensorNetworkDataTabDlg::CopyRow()
{
	if(m_NumOfRows==0)
		return FALSE;
...
BOOL CSensorNetworkDataTabDlg::AddRow()
{
	//static CString
...
BOOL CSensorNetworkDataTabDlg::AddRowForLink(int from_node_name,int to_node_name)
{
	//static CString
	int Index = m_ListCtrl.InsertItem(LVIF_TEXT,m_NumOfRows,NULL,0,0,0,NULL);
...
int CSensorNetworkDataTabDlg::ValidityCheck()
{
	using std::string;
...
CString CSensorNetworkDataTabDlg::GenerateRecordString()
{
	std::string Str;
...
BOOL CSensorNetworkDataTabDlg::DeleteRow()
{
...
void CSensorNetworkDataTabDlg::ZoomToSelectedObject()
{
...
void CSensorNetworkDataTabDlg::OnLvnItemchangedList(NMHDR *pNMHDR, LRESULT *pResult)
{
...
BEGIN_MESSAGE_MAP(CSensorNetworkDataTabDlg, CDialog)
	ON_NOTIFY(LVN_ITEMCHANGED, IDC_GRIDLISTCTRLEX, &CSensorNetworkDataTabDlg::OnLvnItemchangedList)
	ON_NOTIFY(TCN_SELCHANGE, IDC_SETTING_TAB, &CSensorNetworkDataTabDlg::OnTcnSelchangeSettingTab)
END_MESSAGE_MAP()


// CDlgScenarioTab message handlers

void CSensorNetworkDataTabDlg::OnTcnSelchangeSettingTab(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: Add your control notification handler code here
	*pResult = 0;
}
